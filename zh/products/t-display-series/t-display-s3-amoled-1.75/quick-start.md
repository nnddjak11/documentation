...
| `iic_scan` | I2C 设备扫描（ESP-IDF） |
| `touch` | 触摸功能测试（ESP-IDF） |
...
T-Display S3 AMOLED 1.75 支持 LVGL 8.3.5，以 Arduino_DriveBus 驱动显示屏，LVGL 通过 Arduino_GFX 的 `draw16bitRGBBitmap` 作为刷新后端。
...
#define LV_HOR_RES_MAX  280   // 1.75 英寸 AMOLED：280×456
...
`lvgl_benchmark` 示例用于测量该板卡的 LVGL 渲染性能，可作为缓冲区大小调优和绘制速度优化的参考。
