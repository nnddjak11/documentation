...
# T-Display S3 AMOLED 1.64 Quick Start
...
T-Display S3 AMOLED 1.64 supports LVGL 8.3.5. The display is driven via Arduino_DriveBus; LVGL uses Arduino_GFX's `draw16bitRGBBitmap` as the flush backend.
...
#define LV_HOR_RES_MAX  280   // 1.64-inch AMOLED: 280×456
...
The `screen_touch_lvgl_9` example demonstrates a full LVGL 9 touch UI via ESP-IDF. The `lvgl_benchmark` example measures rendering performance. For Arduino LVGL 8 development, use the sketch above and refer to those examples for reference.
...
The charger output is unstable at 5V without a battery. Always connect a battery, or disable the battery channel in software.
